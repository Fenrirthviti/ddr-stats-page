<?php session_start(); $time_start = microtime(true);?>
<!DOCTYPE html>
<html>
<head>
...
if (file_exists('stats.xml')) { 
	...
	for ($i = $start; $i < $end; $i++) {
		...
		list($dir, $songPack, $songName) = explode('/',  $song->attributes()->Dir   );
		echo '<tr>';
		echo '<td class="date">' . $song->Steps->HighScoreList->HighScore->DateTime . '</td>';
		echo '<td class="song" title="Song Pack: ' . $songPack . '">' . $songName . '</td>';
		echo '<td class="difficulty">' . $song->Steps->attributes()->Difficulty . '</td>';
		echo '<td class="score">' . $song->Steps->HighScoreList->HighScore->Score . '</td>';
		...
		echo '<td class="fegdwom">' . $fegdwom . '</td>';
		echo '<td class="plays">' . $song->Steps->HighScoreList->NumTimesPlayed . '</td>';
		echo '<td class="player">' . $profiles[(string) $song->Steps->HighScoreList->HighScore->PlayerGuid] . '</td>'; 		
		echo '</tr>';
	}
	...
	echo '</td>';
	echo '<td class="switch" colspan="4"><a href="../stats-orange.php">Dark Theme</a></td></tr>';
	echo '<td class="render" colspan="8">';
